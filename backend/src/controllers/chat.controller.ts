...
const getCachedHistory = async (chatId: string): Promise<CachedMessage[]> => {
  const now = Date.now();
  ...
  const messages: CachedMessage[] = history.reverse().map(msg => ({
    role: msg.role,
    content: msg.content
  }));
...
const updateCache = (chatId: string, newMessage: { role: string; content: string }): void => {
  const cached = chatCache.get(chatId);
  let messages: CachedMessage[] = cached ? cached.messages : [];
...
export const processMessageLogic = async ({ content, chatId, user, io, socket }: ProcessMessageParams): Promise<any> => {
...
export const getHistory = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    const query: any = { sender: req.user?._id };
    if (chatId) {
      query.chatId = chatId;
    }
