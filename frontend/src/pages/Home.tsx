...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (isNewChatActionRef.current) {
      if (currentChatId) switchChat('null'); // Using string 'null' temporarily to clear chat? 
      // Actually switchChat expects string. Passing 'null' string might be handled in context, 
      // but let's check context. switchChat handles 'null' string by setting null.
      // But cleaner would be if switchChat accepted string | null.
      // In context: const switchChat = useCallback((chatId) => { if (chatId === 'null' ...
      
      // Let's safe cast for now or update context type. Context type says string.
      // Let's pass 'null' as string which context handles specially.
      switchChat('null'); 
